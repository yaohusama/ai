...
class FS_window(QMainWindow, Ui_MainWindow):
        ...
        try:
            ...
            for i in range(nrows):
                for j in range(ncols):#table.at[i, j]
                    self.trainWidget.setItem(i, j, QTableWidgetItem(str(table.at[i, str(j)])))#这里的trainWidget是界面的东西
            ...
            QMessageBox.information(self, 'Warning', '数据为CSV表格', QMessageBox.Ok)
        ...
        try:
            ...
            for i in range(nrows):
                for j in range(ncols):
                    self.testWidget.setItem(i, j, QTableWidgetItem(str(table.at[i, str(j)])))
    ...
    def get_picture(self):
        try:
            ...
            fx = 0.3
            fy = 0.3
            self.image = cv2.resize(self.img, dsize=None, fx=fx, fy=fy, interpolation = cv2.INTER_AREA)
    ...
    def gray_level_co_occurance_matrix(self):
            ...
            for i in range(n):  # 遍历所有特征
                ...
                for j in range(-1, int(numSteps) + 1):
                    for inequal in ['lt', 'gt']:  # 大于和小于的情况，均遍历。lt:less than，gt:greater than
                        ...
                        predictedVals = FS_window.stumpClassify(dataMatrix, i, threshVal, inequal)  # 计算分类结果
        ...
        def adaboost_classifier(self):
            ...
            try:
                ...
                numIt=40
                for i in range(numIt):
                    bestStump, error, classEst = FS_Window.buildStump(self.dataArr, self.classLabels, D)  # 构建单层决策树
                    # print("D:",D.T)
                    alpha = float(0.5 * np.log((1.0 - error) / max(error, 1e-16)))  # 计算弱学习算法权重alpha,使error不等于0,因为分母不能为0
                    bestStump['alpha'] = alpha  # 存储弱学习算法权重
                    weakClassArr.append(bestStump)  # 存储单层决策树
                    # print("classEst: ", classEst.T)
                    expon = np.multiply(-1 * alpha * np.mat(self.classLabels).T, classEst)  # 计算e的指数项
                    D = np.multiply(D, np.exp(expon))
                    D = D / D.sum()  # 根据样本权重公式，更新样本权重
                    # 计算AdaBoost误差，当误差为0的时候，退出循环
                    aggClassEst += alpha * classEst  # 计算类别估计累计值
                    # print("aggClassEst: ", aggClassEst.T)
                    aggErrors = np.multiply(np.sign(aggClassEst) != np.mat(self.classLabels).T, np.ones((m, 1)))  # 计算误差
                    errorRate = aggErrors.sum() / m
                    # print("total error: ", errorRate)
