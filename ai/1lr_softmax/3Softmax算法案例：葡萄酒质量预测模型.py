...
lr = LogisticRegressionCV(fit_intercept=True,max_iter=20000, Cs=np.logspace(-5, 1, 100), 
                          multi_class='multinomial',tol=1e-7, penalty='l2', solver='lbfgs')
...
a = Y_predict == Y_test
print(f"测试集准确率为{a.astype(int).sum() / len(a)}")
## c. 图表展示
